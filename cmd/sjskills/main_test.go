...
func TestExternalPlanApplyAndRestoreContracts(t *testing.T) {
	...
	if !strings.Contains(stdout, `"operation":"plan"`) || !strings.Contains(stdout, `"plan":{"desired"`) || !strings.Contains(stdout, `"action":"install"`) || !strings.Contains(stdout, `"resolved 15 desired skills"`) || strings.Contains(stdout, `"warnings":null`) {
...
func TestExternalPlanMaterializesExpectedContentReadOnly(t *testing.T) {
	...
	if len(projectEnvelope.Warnings) != 0 {
		t.Fatalf("project warnings = %#v, want none for managed dev/go selections", projectEnvelope.Warnings)
	}
	for _, output := range []string{stdout, stderr} {
