...
North star: in production, logs are the primary debugging tool. Select plausible incidents for the area's risk and ask: could you localize the cause from logs alone — no debugger, no repro? Review against those incidents, not against a checklist of log lines.

## Check

- **Decision points, not just failures.** Material operational decisions (fallbacks, retries, degraded modes) need enough structured context to diagnose the selected incidents; do not require a log for every branch or cache hit. A failure log without the preceding decisions is a dead end.
