...
- **Lifecycle hygiene.** Current-state docs describe reality; active plan and TODO sources do not present completed work as remaining work (verify against git history before calling done). Respect an explicit convention that retains completed item files as clearly framed, unlisted history. Target/vision docs are dated or owned enough to tell aspiration from abandonment. Decision records are historical — verify they aren't silently contradicted by code, but do not "update" the records themselves.
