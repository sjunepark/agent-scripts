...
1. Inspect current state with `bunx skills list` for project scope and `bunx skills list -g` for global scope.
   - In this repository, use `skill-registry.json` as the authoritative classification and `scripts/audit-global-skills --profile dev|kicpa` to compare machine-global installs with one explicitly selected profile. The registry-v2 audit is read-only until exact filesystem reconciliation is implemented.
