 ...
 *
 * `pin audit` creates a lightweight /audit skill that redirects to /impeccable-june audit.
...
const ENTRYPOINT_SKILL = 'impeccable-june';

// Marker to identify pins created by this vendored skill (so unpin doesn't delete user or upstream pins)
const PIN_MARKER = '<!-- impeccable-june-pinned-skill -->';
...
/**
 * Find harness skill directories that have this skill installed.
 */
function findHarnessDirs(projectRoot) {
  const dirs = [];
  for (const harness of HARNESS_DIRS) {
    const skillsDir = join(projectRoot, harness, 'skills');
    const entrypointDir = join(skillsDir, ENTRYPOINT_SKILL);
    if (existsSync(entrypointDir) || existsSync(join(skillsDir, `i-${ENTRYPOINT_SKILL}`))) {
      dirs.push(skillsDir);
    }
  }
  return dirs;
...
function generatePinnedSkill(command, metadata) {
  const desc = metadata[command]?.description || `Shortcut for /${ENTRYPOINT_SKILL} ${command}.`;
...
This is a pinned shortcut for \`{{command_prefix}}${ENTRYPOINT_SKILL} ${command}\`.

Invoke {{command_prefix}}${ENTRYPOINT_SKILL} ${command}, passing along any arguments provided here, and follow its instructions.
`;
}
...
function pin(command, projectRoot) {
  ...
  if (harnessDirs.length === 0) {
    console.log(`No harness directories with ${ENTRYPOINT_SKILL} installed found.`);
    return false;
  }
...
function unpin(command, projectRoot) {
  ...
  if (removed > 0) {
    ...
    console.log(`Use /${ENTRYPOINT_SKILL} ${command} to access it.`);
  } else {
