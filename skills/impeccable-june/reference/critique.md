...
Once the report above is finalized, write it to `.impeccable/critique/` so the user can refer back, and so `{{command_prefix}}impeccable-june polish` can pick up the priority issues without a copy-paste.
...
- End with `{{command_prefix}}impeccable-june polish` as the final step if any fixes were recommended

After presenting the summary, tell the user:

> You can ask me to run these one at a time, all at once, or in any order you prefer.
>
> Re-run `{{command_prefix}}impeccable-june critique` after fixes to see your score improve.
