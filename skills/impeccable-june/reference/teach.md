...
- **PRODUCT.md exists, DESIGN.md missing**: skip to Step 5 and offer to run `/impeccable-june document` for DESIGN.md.
...
If teach was invoked as a setup blocker by another command, such as `{{command_prefix}}impeccable-june craft landing page`, pause that command here. Complete teach, re-run the loader, then resume the original command with the freshly loaded context. For craft, resume into shape next; teach creates project context, but it is not a substitute for the task-specific shape interview and confirmed design brief.
...
Offer `/impeccable-june document` either way. Two paths:
...
If the user agrees, delegate to `/impeccable-june document` (it auto-detects scan vs seed). Load its reference and follow that flow.

If the user prefers to skip, mention they can run `/impeccable-june document` any time later.
...
If teach was invoked as a blocker by another impeccable command (e.g. the user ran `/impeccable-june polish` with no PRODUCT.md), resume that original task now with the fresh context.
