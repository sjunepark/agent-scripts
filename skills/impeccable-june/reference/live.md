...
Before booting live mode, establish the app URL as an invariant. Browser-driven design work must target the page that loads the files in `.impeccable/live/config.json`, not whichever dev server owns a framework's default port.

1. Prefer a repo-specific strict command such as `bun run dev:live`, `pnpm dev:live`, or `npm run dev:live`. Strict ports are better than auto-incrementing ports because wrong-app failures are worse than an early port conflict.
2. If no live command exists, inspect `package.json`, dev scripts, recent dev logs, or terminal output and identify the exact app URL. For Vite/SvelteKit, do not assume `http://localhost:5173/`; Vite may silently move to 5174+ when another app is running.
3. If a project keeps dev logs, use them as evidence. Example: a log saying `Local: http://localhost:5176/` beats any framework default.
4. The `serverPort` returned by `live.mjs` is the Impeccable helper server for `/live.js`, SSE, and `/poll`. Never open it as the app page.
5. If live mode was interrupted or points at the wrong app, run `live-server.mjs stop` before retrying unless you intentionally need to resume a durable session.

Recommended repo convention for smooth live sessions:

- `PRODUCT.md` exists and is specific.
- `DESIGN.md` exists when code already has a visual system.
- `.impeccable/live/config.json` is committed.
- `.impeccable/live/server.json` and `.impeccable/live/sessions/` are ignored.
- A strict live dev command, ideally `dev:live`, prints or implies one stable URL.

Common failure signatures:

- `live-poll.mjs` printing `Working...` usually means it is waiting for a browser event, not that it has crashed.
- `browser open http://localhost:5173/` is suspicious in any repo with a different strict live URL or recent Vite port-skipping logs.
- `browser snapshot -i` reporting `0 interactive elements` is not fatal for visual live mode.
- `agent-browser skills get core` with `Unknown command: skills` is likely browser CLI compatibility noise unless later browser operations fail.
- Stale `impeccable-live-start`, `impeccable-carbonize-start`, or `impeccable-variants-start` markers in source mean cleanup was interrupted and must be completed before more live work.
...
1. `live.mjs`: boot.
2. Navigate to the URL that serves `pageFile`. Prefer the repo's strict live dev command and confirmed dev output over framework defaults. If you can't infer the URL confidently, tell the user once to open their dev/preview URL. Never use `serverPort` as that URL; it's the helper, not the app.
