...
3. Delete local branches with `git branch -d -- <branch>`. If it refuses, inspect
   the reason. Use `-D` only for the individually proven integrated tip when the
   refusal is due to ancestry/upstream checks; never use it to bypass occupancy
   or unresolved work. Do not treat a published feature upstream as the durable
   integration target merely because `-d` accepts it.
