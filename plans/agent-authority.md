# Agent authority

Status: source edits, bounded review, scoped documentation harmonization, and
validation, publication, and project reconciliation complete for the initial
authority edits. The sjskills follow-up has passed bounded review, scoped documentation
harmonization, skill validation, and the Node and Go suites; publication and
configured reconciliation are next.

Scope: consolidate task authority in the Codex, Claude, and Pi global defaults;
remove redundant confirmation from `next-goal`, `create-pr`, and the macOS and
Windows cleanup skills, including their runtime references and live evaluations.

- Current and earlier requests, implied intermediate work, and delegated choices
  supply authority. Ask when material targets or consequences remain unresolved.
- Preserve explicit inspection limits, unrelated work, protected data, technical
  prerequisites, tool permissions, and boundaries outside the selected task.
- Updated evaluation cases cover initial selections, delegated choices, scoped
  cleanup, and explicit requests to wait. Historical evaluation reports remain
  records of their original runs.
- Bounded review found and corrected stale evaluation wording. Repository skill
  validation, evaluation JSON checks, and diff whitespace checks pass. The legacy
  skill-creator validator rejects the macOS skill's pre-existing `compatibility`
  field; the repository validator accepts it.
- An independent trial in an isolated repository selected Workspace Import and
  emitted a complete goal contract carrying delegated product decisions without
  another confirmation. The fixture remained unchanged. Cleanup behavior received
  instruction review; no live cleanup trial was performed.
- Published source commit `731c74d` to `main` and verified every changed skill
  tree against the registry source before applying the committed `dev` and `go`
  project profiles. Both `create-pr` placements updated; the final plan reports
  all placements unchanged with no warnings or blocks.
- Retain project quarantine `9b95c14d0107e91ca8e71924429afaa3` through a normal
  work cycle. Restore is a separate operation.
- The follow-up extends sync authority to the configured project manifest and
  fixed global baseline. The agent prepares and reviews required evidence;
  provenance, conflict, and filesystem checks remain mandatory. Publication and
  completion of the pending global updates are next. Cleanup skills remain
  outside this project selection and the fixed global baseline.
- The Codex, Claude, and Pi global instruction symlinks resolve to the revised
  sources; active-session reloading remains unverified. See
  [settings sync](../docs/settings-sync.md) for installation boundaries.
