---
name: sync
description: Commit and push intended skill changes, then reinstall or update selected published skills from the current repository onto the current machine with the skills CLI, using the repository's remote URL rather than a local path. Use when the user wants to sync, bootstrap, reinstall, or update repo skills from GitHub or another git remote, not `./skills` or the working tree.
---
...
- For a machine-global Claude Code + Pi install, target selected skills by name with copy mode: `bunx skills add "<skills-subpath-url>" --skill <skill-name> --copy -g -a claude-code -a pi -y`.
...
- Default to installing selected skills for Claude Code + Pi with `--skill <skill-name> --copy -g -a claude-code -a pi -y` unless the user explicitly wants another target set.
- If the user wants `skills list -g` to stay scoped to Claude Code + Pi, do not leave the selected repo skills in `~/.agents/skills`.
...
git status --short
git add skills/manual-branch-integrator/SKILL.md
...
- "Sync selected skills from this repo onto this machine."
- "Commit, push, and reinstall the repo skills from GitHub."
- "Install this repo skill from GitHub, not the local path."
